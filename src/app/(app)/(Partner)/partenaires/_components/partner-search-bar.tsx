...
export function SearchBar() {
    ...
    const fetchResults = useCallback(async (searchTerm: string) => {
        if (searchTerm) {
            setIsSearching(true);
            try {
                const response = await fetch(`/api/partners?search=${searchTerm}&limit=5`);
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                const partners = await response.json();
                setResults(partners.data);
            } catch (error) {
                    ...
                    {isSearching ? (
                        ...
                        results.map((partner) => (
                            ...
                            >
                                ...
                                <div className="text-sm text-gray-600">{typeLabels[partner.type]}</div>
                            </div>
                        ))
                    ) : (
